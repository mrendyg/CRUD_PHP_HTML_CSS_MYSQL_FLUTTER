...
try {
  ...
  $id = $_GET['id'];
  $consultaSQL = "SELECT * FROM padre_estudiante WHERE id_alumno =" . $id;
...
if (isset($padre) && $padre) {
  ...
  <div class="container">
    <div class="row">
      <div class="col-md-12">
        <h2 class="mt-4">Editar padre <?= escapar($padre['nombre']) . ' ' . escapar($padre['apellido_paterno'])  ?></h2>
        <hr>
        <form method="post">
          <div class="form-group">
            <label for="nombre">Nombre</label>
            <input type="text" name="nombre" id="nombre" value="<?= escapar($padre['nombre']) ?>" class="form-control">
          </div>
          <div class="form-group">
          ...
          <div class="form-group">
            ...
            <input type="number" name="CI" id="CI" value="<?= escapar($padre['CI']) ?>" class="form-control">
          </div>
          <div class="form-group">
            <label for="peso">Peso</label>
            <input type="number" name="peso" id="peso" value="<?= escapar($padre['peso']) ?>" class="form-control">
          </div>
          <div class="form-group">
            <label for="vacunas_al_dia">Vacunas al dia</label>
            <input type="text" name="vacunas_al_dia" id="vacunas_al_dia" value="<?= escapar($padre['vacunas_al_dia']) ?>" class="form-control">
          </div>
